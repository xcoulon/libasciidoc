...
var _ = Describe("attributes", func() {
	...
	Context("inline attributes", func() {
		...
		// Skipping: not sure if such spaces should be allowed in inline attributes,
		// since they aren't in block attributes
		XIt("block image with double quoted alt extra whitespace", func() {
		...
		// Skipping: not sure if such spaces should be allowed in inline attributes,
		// since they aren't in block attributes
		XIt("block image with single quoted alt and embedded quotes", func() {
		...
		// Skipping: not sure if such spaces should be allowed in inline attributes,
		// since they aren't in block attributes
		XIt("block image with single quoted alt extra whitespace", func() {
			source := "image::foo.png[ 'This \\Backslash  2Spaced End Space ' ]"
			expected := types.DraftDocument{
				Elements: []interface{}{
					types.ImageBlock{
